...
import { RpHttpService } from './rp-http.service';
...
export class RpReferences {
  ...
  getV6Location(cb: any) {
    let url: string = this.ics.getApiUrl() + 'main/locations/' + this.ics._profile.userSk;
    this.http.doGet(url).subscribe(
      data => {
        this._lov3.locdatas = data;
        var locations = '';
        data.forEach(function (l, i) { if (i > 0) locations += ','; locations += l.code; });
        this.ics._locationRight = locations;
        if (cb) cb.call();
      },
      () => { if (cb) cb.call(); }
    );
  ...
  getV6BinData(cb: any) {
    let url: string = this.ics.getApiUrl() + 'common/get-bins';
    this.http.doGet(url).subscribe(
      data => { this._lov3.bin_datas = data; if (cb) cb.call(); },
      () => { if (cb) cb.call(); }
    );
  }
} 